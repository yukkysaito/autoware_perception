...
void Yolo3DetectorNode::convert_rect_to_image_obj(std::vector< RectClassScore<float> >& in_objects, autoware_msgs::DynamicObjectWithFeatureArray& out_message)
{
    ...
    {
        {
            ...
            obj.feature.roi.x_offset = (in_objects[i].x /image_ratio_) - image_left_right_border_/image_ratio_;
            obj.feature.roi.y_offset = std::max((int)(((double)in_objects[i].y /image_ratio_) - (double)image_top_bottom_border_/image_ratio_), 0);
            obj.feature.roi.width = in_objects[i].w /image_ratio_;
            obj.feature.roi.height = in_objects[i].h /image_ratio_;
            if (in_objects[i].x < 0)
            ...
            obj.object.semantic.confidence = in_objects[i].score;
            
            if (in_objects[i].GetClassString() == std::string("car"))
            {
                obj.object.semantic.type = autoware_msgs::Semantic::CAR;
            }
            else if (in_objects[i].GetClassString() == std::string("person"))
            {
                obj.object.semantic.type = autoware_msgs::Semantic::PEDESTRIAN;
            }
            else if (in_objects[i].GetClassString() == std::string("bus"))
            {
                obj.object.semantic.type = autoware_msgs::Semantic::BUS;
            }
            else if (in_objects[i].GetClassString() == std::string("truck"))
            {
                obj.object.semantic.type = autoware_msgs::Semantic::TRUCK;
            }
            else if (in_objects[i].GetClassString() == std::string("bicycle"))
            {
                obj.object.semantic.type = autoware_msgs::Semantic::BICYCLE;
            }
            else if (in_objects[i].GetClassString() == std::string("motorbike"))
            {
            ...
            out_message.feature_objects.push_back(obj);
...
void Yolo3DetectorNode::image_callback(const sensor_msgs::ImageConstPtr& in_image_message)
{
    ...
    autoware_msgs::DynamicObjectWithFeatureArray output_message;
...
void Yolo3DetectorNode::Run()
{
    ...
    publisher_objects_ = node_handle_.advertise<autoware_msgs::DynamicObjectWithFeatureArray>("rois", 1);
